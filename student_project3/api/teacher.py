...
def get_all_teachers(db: Session = Depends(get_db)):
    ...
    :return: 教师列表
    """
    teachers = TeacherService.get_all_teachers(db)
...
def add_teacher(t: TeacherCreate, db: Session = Depends(get_db)):
    ...
    :return: 创建结果
    """
    result = TeacherService.create_teacher(db, t)
...
def list_teachers(
    ...
    :return: 分页教师数据
    """
    total, data = TeacherService.get_teachers(db, teacher_name, gender, page, page_size)
    return {
        "code": 200,
        "message": "查询成功",
        "total": total,
...
def get_teacher_by_id(teacher_id: int, db: Session = Depends(get_db)):
    ...
    :return: 教师信息
    """
    result = TeacherService.get_teacher(db, teacher_id)
...
def update_teacher_api(teacher_id: int, data: TeacherUpdate, db: Session = Depends(get_db)):
    ...
    :return: 修改结果
    """
    result = TeacherService.update_teacher(db, teacher_id, data)
...
def delete_teacher(teacher_id: int, db: Session = Depends(get_db)):
    ...
    :return: 删除结果
    """
    TeacherService.delete_teacher(db, teacher_id)
...
def list_deleted_teachers(
    ...
    :return: 已删除教师数据
    """
    total, data = TeacherService.get_deleted_teachers(db, page, page_size)
    return {
        "code": 200,
        "message": "查询成功",
        "total": total,
...
def restore_teacher_api(teacher_id: int, db: Session = Depends(get_db)):
    ...
    :return: 恢复结果
    """
    result = TeacherService.restore_teacher(db, teacher_id)
...
def get_stats(db: Session = Depends(get_db)):
    ...
    :return: 统计数据
    """
    result = TeacherService.get_stats(db)
    return {"code": 200, "message": "查询成功", "data": result}
