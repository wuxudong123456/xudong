...
async def add_student(s: StudentCreate, db: AsyncSession = Depends(get_db)):
    """
    新增学生信息
    ...
    :return: 创建结果
    """
    result = await StudentService.create_student_service(db, s)
...
async def list_students(
    ...
    :return: 分页学生数据
    """
    total, data = await StudentService.get_students(
        db, student_name, class_id, page, page_size
    )
    return {
        "code": 200,
        "message": "查询成功",
...
async def get_one_student(student_id: int, db: AsyncSession = Depends(get_db)):
    """
    根据ID查询单个学生信息
    ...
    :return: 学生信息
    """
    result = await StudentService.get_student(db, student_id)
...
async def update_student_info(
    ...
    :return: 更新结果
    """
    result = await StudentService.update_student(db, student_id, s)
...
async def restore_student_info(student_id: int, db: AsyncSession = Depends(get_db)):
    """
    恢复已删除的学生
...
async def check_deleted_students(
    ...
    :return: 已删除学生数据
    """
    total, data = await StudentService.get_deleted_students(
        db, student_name, page, page_size
    )
    return {
        "code": 200,
        "message": "查询成功",
...
async def check_student_age(
    ...
    :return: 符合条件的学生列表
    """
    result = await StudentService.get_students_over_age(db, age_threshold)
    return {"code": 200, "message": "查询成功", "data": result}


@router.get("/check_gender", response_model=dict, summary="统计班级性别分布")
async def check_student_gender(
    class_id: Optional[int] = Query(None, description="班级ID"),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    :return: 性别统计数据
    """
    result = await StudentService.get_student_gender_stats(db, class_id)
    return {"code": 200, "message": "查询成功", "data": result}
