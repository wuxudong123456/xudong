...
def get_all_classinfo_api(db: Session = Depends(get_db)):
    ...
    :return: 班级列表
    """
    res = ClassInfoService.get_all_classinfo_service(db)
...
def get_one_class_api(class_id: int, db: Session = Depends(get_db)):
    ...
    :return: 班级信息
    """
    res = ClassInfoService.get_one_classinfo_service(db, class_id)
    return {"code": 200, "message": "查询成功", "data": res}


@class_router.post("/add", response_model=dict, summary="添加班级")
def add_class_api(cls: ClassCreate, db: Session = Depends(get_db)):
    """
    添加班级信息
    ...
    :return: 创建结果
    """
    result = ClassInfoService.post_add_class_service(db, cls)
...
def put_update_class(class_id: int, update_data: ClassUpdate, db: Session = Depends(get_db)):
    ...
    :return: 修改结果
    """
    result = ClassInfoService.put_update_class_service(db, class_id, update_data)
...
def delete_class_api(class_id: int, db: Session = Depends(get_db)):
    ...
    :return: 删除结果
    """
    ClassInfoService.delete_class_service(db, class_id)
...
def restore_class_api(class_id: int, db: Session = Depends(get_db)):
    ...
    :return: 恢复结果
    """
    result = ClassInfoService.restore_class_service(db, class_id)
    return {"code": 200, "message": "恢复成功", "data": result}


@class_router.get("/count/month", response_model=dict, summary="统计每月班级数")
def count_class_month(month: Optional[str] = Query(None, description="月份（格式：YYYY-MM）"), db: Session = Depends(get_db)):
    """
    统计每个月的班级数量

    :param month: 月份（可选，不传则统计所有月份）
    :param db: 数据库会话
    :return: 统计数据
    """
    result = ClassInfoService.count_class_month_service(db, month=month)
...
def get_class_by_lecturer_id_api(lecturer_id: int, db: Session = Depends(get_db)):
    ...
    :return: 班级列表
    """
    result = ClassInfoService.get_class_by_lecturer_id_service(db, lecturer_id)
    return {"code": 200, "message": "查询成功", "data": result}
