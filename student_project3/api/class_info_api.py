...
def add_class_api(cls: ClassCreate, db: Session = Depends(get_db)):
    ...
    :return: 创建结果
    """
    result = post_add_class_service(db, cls)
...
def put_update_class(class_id: int, update_data: ClassUpdate, db: Session = Depends(get_db)):
    ...
    :return: 修改结果
    """
    result = put_update_class_service(db, class_id, update_data)
...
def restore_class_api(class_id: int, db: Session = Depends(get_db)):
    ...
    :return: 恢复结果
    """
    result = restore_class_service(db, class_id)
...
def count_class_month(month: Optional[str] = Query(None, description="月份（格式：YYYY-MM）"), db: Session = Depends(get_db)):
    ...
    :return: 统计数据
    """
    result = count_class_month_service(db, month=month)
...
def get_class_by_lecturer_id_api(lecturer_id: int, db: Session = Depends(get_db)):
    ...
    :return: 班级列表
    """
    result = get_class_by_lecturer_id_service(db, lecturer_id)
    return {"code": 200, "message": "查询成功", "data": result}