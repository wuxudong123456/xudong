...
async def add_score(score: Score_QQ, db: AsyncSession = Depends(get_db)):
    """
    添加单条成绩信息
    ...
    :return: 添加结果
    """
    result = await ScoreService.add_score_service(db, score)
...
async def get_scores(
    ...
    :return: 分页成绩数据
    """
    result = await ScoreService.get_scores_service(db, id, student_no, exam_order, page, size)
...
async def update_score(
    ...
    :return: 修改结果
    """
    data = await ScoreService.update_score_service(db, score_id, update_data)
...
async def delete_score(score_id: int, db: AsyncSession = Depends(get_db)):
    ...
    :return: 删除结果
    """
    await ScoreService.delete_score_service(db, score_id)
...
async def restore_score(
    ...
    :return: 恢复结果
    """
    count = await ScoreService.restore_score_service(db, id, student_no, exam_order)
    return {"code": 200, "message": f"恢复成功，共恢复 {count} 条", "data": count}


@router.get("/all-above-80", response_model=dict, summary="查询80分以上学生")
async def all_above_80(db: AsyncSession = Depends(get_db)):
    """
    查询所有科目80分以上的学生
    ...
    :return: 符合条件的学生列表
    """
    data = await ScoreService.get_all_above_80_service(db)
    return {"code": 200, "message": "查询成功", "data": data}


@router.get("/multiple-fail", response_model=dict, summary="查询不及格超过2次的学生")
async def multiple_fail(db: AsyncSession = Depends(get_db)):
    """
    查询不及格次数超过2次的学生
    ...
    :return: 符合条件的学生列表
    """
    data = await ScoreService.get_multiple_fail_service(db)
    return {"code": 200, "message": "查询成功", "data": data}
...
async def class_avg(
    ...
    :return: 班级平均分统计
    """
    data = await ScoreService.get_class_avg_service(db, class_id)
    return {"code": 200, "message": "查询成功", "data": data}
